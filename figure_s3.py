...
PARAMS = {
    ...
    "extrapolate_yr": xr.DataArray(
        [2024], [[2024]], ["year"]
    ),  # year which want to pivot the main UNSEEN distribution on (i.e. all panels apart from func-time panels)
    "n_iterations": 1000,
    "trend_source": "model",
    "beyond_hindcast_yrs": xr.DataArray(
        np.arange(2016, 2024 + 1), [np.arange(2016, 2024 + 1)], ["year"]
    ),  # required for calculating probabilities as function of time (/trend) if extrapolated = True
}
...
unseen_distr, _ = un.get_unseen_distr(
    ...
    extr_year=PARAMS["extrapolate_yr"],
)
...
# get probabilities as function of time (/trend)
ft_risk, ft_risk_extrapolated = un.get_functime_risk(
    model_da=large_da,
    trend_data=large_da,
    extr_avg_period=UNSEENConfig.extr_avg_period,
    focus_event=june2023_peak,
    core_pivot_yrs=large_da["year"],
    trend_source=PARAMS["trend_source"],
    extrapolated=extrapolated,
    extrapolate_yrs=PARAMS["beyond_hindcast_yrs"],
)

# get upper and lower 95% CI level for ft_risk
...
# plt.savefig("plot_images/figure_s3.png", dpi=300, bbox_inches="tight")
